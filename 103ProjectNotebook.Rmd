...
gene: CLEC3B
continuous: gene expression, ventilator.free_days, age
categorical: charlson binary, mechanical ventilation, sex

```{r}
install.packages("kableExtra") # we will work with the kableExtra package
```

Code adapted from class: https://canvas.dartmouth.edu/courses/59947
...
I will make a violin plot since we haven't made one in class yet

```{r}
violin_plot <- ggplot(subset, aes(x = gene_expression_data, y = charlson_binary, fill = mechanical_ventilation)) + # pick variables to go in the plot
  geom_violin() + # make it a violin plot
  labs(title="CLEC3B Gene Expression vs. Charlson Binary", x="Gene Expression", y="Charlson Binary", fill = "Mechnical Ventilation") + # labels
  scale_y_discrete(breaks = c(FALSE, TRUE), labels = c("<5", ">5")) + # I want the values to be reflected in the text
  scale_fill_manual(values = c("blue", "orange")) + # colors for the mechanical ventilation
  theme_minimal() # theme
