...
I will make a Marginal Histogram plot since we haven't made one in class yet:

```{r}
install.packages("ggExtra") # uses ggExtra
```
...
Two categorical variables for the 10 genes: I selected icu status and sex

step 1. subset df to just 10 genes

```{r}
#tengenes <- expressiondf[1:10, ] # select just 10 genes from the expression/original dataset
#ten_genes <- data.frame(t(tengenes)) # transpose
#colnames(ten_genes) <- tengenes$X # make column names the genes
#ten_genes <- ten_genes[-1, ] # remove the first row because we already have the gene names as the column names
#ten_genes
```
...
step 2: c bind ten_genes with metadf
