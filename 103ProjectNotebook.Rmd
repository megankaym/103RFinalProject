...
4. develop a function that will create these plots with input as: 
  ...
  For this function, I decided to plot the data for all the genes of interest (since it is not specified in the assignment) so there are 3 plots no matter how many genes you put in. I make a second function below this that makes 3 plots per gene you put in.
...
```{r}
my_fun <- function(df, genes, contvar, list2catvars){
  for (gene in genes){
  ...
  # box plot
    boxplot <- ggplot(completedf, aes(x = unlist(completedf[contvar]), y = unlist(completedf[list2catvars[1]]), color = unlist(completedf[list2catvars[2]]))) + #create a boxplot of all 3 vars
      geom_boxplot() +
      geom_jitter() +
      labs(title = substitute(paste(italic(gene), contvar, " with ", list2catvars)), x = contvar, y = list2catvars[1]) + 
      theme(plot.title = element_text(colour = "orangered", hjust = 0.18)) + # for the plot title
      scale_fill_manual(labels=c("<5", ">5")) +
      coord_flip() # name the x axis by ><5
    print(boxplot)
...
```{r}
my_fun(df, c('CLEC3B', 'APOA1'), "gene_expression_data", c("charlson_binary", "mechanical_ventilation"))
```
